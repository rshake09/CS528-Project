import cv2
import numpy as np
import mediapipe as mp
import time
from tensorflow import keras

# ---------------- LOAD MODEL ----------------
model = keras.models.load_model("../models/asl_model.keras")

# ⚠️ IMPORTANT: update this to match your training classes
gesture_labels = ["HELLO", "GOODBYE", "THANK YOU", "HUNGRY", "A", "B", "C", "D"]

print("[INFO] Model loaded successfully")
print(f"[INFO] Classes: {gesture_labels}")

# ---------------- SETUP ----------------
mp_hands = mp.solutions.hands
mp_draw = mp.solutions.drawing_utils

hands = mp_hands.Hands(
    static_image_mode=False,
    max_num_hands=1,
    min_detection_confidence=0.7,
    min_tracking_confidence=0.5
)

cap = cv2.VideoCapture(0)

# ---------------- STATE ----------------
sentence = []
current_prediction = None
hold_start = None
progress = 0.0

HOLD_DURATION = 1.5
last_confirmed = None
last_confirmed_time = 0
COOLDOWN = 2.0

print("[INFO] Running...")
print("Press 'c' to clear | 'q' to quit")

# ---------------- LOOP ----------------
while True:
    ret, frame = cap.read()
    if not ret:
        break

    frame = cv2.flip(frame, 1)
    h, w, _ = frame.shape

    image_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    result = hands.process(image_rgb)

    prediction = None
    confidence = 0.0

    # ---------------- HAND DETECTION ----------------
    if result.multi_hand_landmarks:
        hand_landmarks = result.multi_hand_landmarks[0]
        mp_draw.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS)

        wrist_x = hand_landmarks.landmark[0].x
        wrist_y = hand_landmarks.landmark[0].y

        features = []
        for lm in hand_landmarks.landmark:
            features.append(lm.x - wrist_x)
            features.append(lm.y - wrist_y)

        features = np.array(features).reshape(1, -1)

        # ---------------- MODEL PREDICTION ----------------
        probs = model.predict(features, verbose=0)[0]
        class_index = np.argmax(probs)

        prediction = gesture_labels[class_index]
        confidence = probs[class_index]

        # filter low confidence
        if confidence < 0.6:
            prediction = None

    # ---------------- HOLD LOGIC ----------------
    now = time.time()

    if prediction is not None:
        if prediction == current_prediction:
            if hold_start is None:
                hold_start = now

            held = now - hold_start
            progress = min(held / HOLD_DURATION, 1.0)

            if held >= HOLD_DURATION:
                if prediction != last_confirmed or (now - last_confirmed_time) > COOLDOWN:
                    sentence.append(prediction)
                    last_confirmed = prediction
                    last_confirmed_time = now
                    hold_start = now
        else:
            current_prediction = prediction
            hold_start = now
            progress = 0.0
    else:
        current_prediction = None
        hold_start = None
        progress = 0.0

    # ---------------- UI ----------------
    overlay = frame.copy()
    cv2.rectangle(overlay, (0, 0), (w, 70), (20, 20, 20), -1)
    cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)

    sentence_text = " ".join(sentence) if sentence else "..."
    cv2.putText(frame, sentence_text, (10, 48),
                cv2.FONT_HERSHEY_SIMPLEX, 1.1, (255, 255, 255), 2)

    if prediction:
        label = f"{prediction} ({int(confidence * 100)}%)"
        cv2.putText(frame, label, (10, h - 80),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)

        bar_w = int(w * progress)
        cv2.rectangle(frame, (0, h - 20), (bar_w, h), (0, 255, 0), -1)
        cv2.rectangle(frame, (0, h - 20), (w, h), (100, 100, 100), 2)

        hint = "Hold..." if progress < 1.0 else "Added!"
        cv2.putText(frame, hint, (10, h - 35),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
    else:
        cv2.putText(frame, "No gesture detected", (10, h - 80),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (150, 150, 150), 2)

    cv2.putText(frame, "C: clear | Q: quit", (w - 200, h - 35),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (150, 150, 150), 1)

    cv2.imshow("ASL Gesture Recognition", frame)

    key = cv2.waitKey(1)
    if key == ord('q'):
        break
    elif key == ord('c'):
        sentence = []
        print("[INFO] Sentence cleared")

cap.release()
cv2.destroyAllWindows()