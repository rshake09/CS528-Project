// code to add later to espidf project
...
/* ─────────────────────────────────────────────────────────
 * Main task: read + print CSV at ~100 Hz
 * ───────────────────────────────────────────────────────── */
static void imu_stream_task(void *arg)
{
    ...
    printf("timestamp_ms,ax_g,ay_g,az_g,gx_dps,gy_dps,gz_dps\n");
    fflush(stdout);

    while (1) {
        esp_err_t err = mpu6050_read(&s);

        if (err == ESP_OK) {
            int64_t ts_ms = esp_timer_get_time() / 1000;
            // CSV line — 3 decimal places is plenty for gesture work
...
void app_main(void)
{
    ESP_LOGI(TAG, "CS528 Air Drawing — Milestone 1");
    i2c_master_init();
    mpu6050_init();
